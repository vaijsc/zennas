...
from latency import predict_latency
import ipdb
...
def get_new_random_structure_str(AnyPlainNet, structure_str, num_classes, get_search_space_func,
    ...
    for replace_count in range(num_replaces):
        ...
        # Choose different layer to modify
        ...
        if len(new_student_block_str) > 0:
            ...
            new_student_block = new_student_block[0] # Get it out from the list of one element
            if random_id > 0:
...
def main(args, argv):
    ...
    for i in range(11):
        ...
        while True:
            random_structure_str = get_new_random_structure_str(
                    AnyPlainNet=AnyPlainNet, structure_str=random_structure_str, num_classes=args.num_classes,
                    get_search_space_func=select_search_space.gen_pruned_search_space, num_replaces=2)
            ...
            if args.budget_flops is not None:
                ...
                print(args.budget_flops, the_model_flops)
                if args.budget_flops * 0.8 < the_model_flops:
                    continue

            the_latency = np.inf
            if args.budget_latency is not None:
                the_latency = get_latency(AnyPlainNet, random_structure_str, gpu, args)
                # the_latency = predict_latency(AnyPlainNet, random_structure_str, gpu, args)
