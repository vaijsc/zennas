...
save_dir=./save_dir/Zen_NAS_ImageNet_flops300M
mkdir -p ${save_dir}
...
epochs=2
warmup=1
evolution_max_iter=1000  # we suggest evolution_max_iter=480000 for sufficient searching
...
python ts_train_image_classification.py --dataset imagenet --num_classes 1000 \
  ...
  --input_image_size ${resolution} --epochs ${epochs} --warmup ${warmup} \
  ...
  --batch_size_per_gpu 64 --save_dir ${save_dir}/ts_effnet_b3ns_epochs${epochs} --auto_resume