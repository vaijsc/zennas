...
class BaseLearner(object):
    ...
    def eval_task(self):
        y_pred, y_pred_with_task, y_pred_given_task, y_true, y_pred_task, y_true_task = self._eval_cnn(self.test_loader)
        cnn_accy = self._evaluate(y_pred, y_true)
        cnn_accy_with_task = self._evaluate(y_pred_with_task, y_true)
        cnn_accy_given_task = self._evaluate(y_pred_given_task, y_true)
        ...
        return cnn_accy, cnn_accy_with_task, cnn_accy_given_task, nme_accy, cnn_accy_task
    
    def eval_tasktest(self, args):
        y_pred, y_pred_softmax, y_pred_with_task, y_pred_given_task, y_true, y_pred_task, y_true_task = self._eval_cnn_test(self.test_loader, args)
        cnn_accy = self._evaluate(y_pred, y_true)
        cnn_accy_softmax = self._evaluate(y_pred_softmax, y_true)
        cnn_accy_with_task = self._evaluate(y_pred_with_task, y_true)
        cnn_accy_given_task = self._evaluate(y_pred_given_task, y_true)
        cnn_accy_task = (y_pred_task == y_true_task).sum().item()/len(y_pred_task)
        ...
        return cnn_accy, cnn_accy_softmax, cnn_accy_with_task, cnn_accy_given_task, nme_accy, cnn_accy_task
